...
bool cUtils::test_exact_stringMatch_in_list(QString str, QStringList sStrList)
{
    bool ok = sStrList.contains(str);
    ...
    if(ok)
    {
        ...
        int N = str.count();
        int M = sStrList.count();

        for(int i=0;i<M;i++)
        {
            QString s = sStrList[i];
            int a = s.count();
