...
vector<Vec3f>  cOpenCv_Utils::get_circles(Mat &src, float min_size, float max_size)
{
    vector<Vec3f> our_circles;
    Vec3f circle;

    vector<Vec3f> circles = get_circles(src);


    // find only circles detected within our range of interest
    for( size_t i = 0; i < circles.size(); i++ )
    {
        //Point center(cvRound(circles[i][0]), cvRound(circles[i][1]));  //cvRound(); rounds floating pt to nearest int
        float radius = circles[i][2];

        if((radius >= min_size)&&(radius <= max_size))
        {
            circle = circles[i];
            our_circles.push_back(circle);
        }
    }
...
vector<Vec3f>  cOpenCv_Utils::get_circles(Mat &src)
{
    Mat gray;
    ...
    vector<Vec3f> circles;  // circles[i_th circle]([centerX][centerY][radius])
    ...
    for( size_t i = 0; i < circles.size(); i++ )
    {
        ...
        circle( src, center, 2, Scalar(255,255,0), -1, 8, 0 );// circle center
        circle( src, center, radius, Scalar(255,0,0), 2, 8, 0 );// circle outline
     }
